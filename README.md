...
会话建立时手机B 会上报屏幕尺寸与解码能力,电脑据此决定怎么编码:

- **编码**:两端都支持硬件 H.265 就用 H.265(同画质约省 30% 码率),否则回落 H.264。`video_codec: "h264"` 可强制。
- **分辨率**:默认 `auto` —— 按**手机B 的屏幕**编码,而不是被控手机的原生分辨率(后者等于把码率花在看不见的像素上)。`resolution: "original"` 改回原始分辨率。
- **码率**:跟着分辨率与路径走。局域网直连给足画质(上限 12 Mbps),走公网中继时以配置里的 `bit_rate` 为上限。
- **静音不传输**:AAC 是恒定码率,静音也会编满(实测常驻 121 kbps,占静止画面总流量的 72%)。电脑端靠"编码器输出在极少数几种字节序列间循环"这一特征识别静音并停发,实测**抑制 99%**;一有声音立刻恢复,不会切掉起音。
- **声音开关**:播放页右上角喇叭键,关掉后电脑端直接停发,省的是链路流量。

## 链路指标与自适应

播放页右上角的 **ⓘ** 打开指标面板:码率、帧率、网络 RTT(P50/P95)、收到→上屏耗时、积压与丢弃、累计流量。

指标是这样量的:手机A、电脑、手机B 三个时钟互不同步,跨设备时间戳不能直接相减。所以拆成两段各自在单一时钟域内测——**网络段**用观看端发、电脑原样回弹的时间戳算 RTT;**本机段**从"收到该帧"到"该帧上屏",全程用手机B 自己的时钟。面板上写的是「链路+解码」而不是「端到端」,因为手机A 的采集与编码耗时量不到,不能算进去。

局域网实测:RTT P50 7ms,收到→上屏 P50 17ms。

**自适应码率**:观看端每 2 秒上报 RTT P95、丢弃数、积压给电脑,持续拥塞就降一档,稳定 60 秒才升一档,两次调整间隔至少 20 秒。scrcpy-server 不支持运行中改码率,所以换档要重启编码器——电脑会先发一个重置信号,观看端据此重建解码器,**观看端连接与触摸控制全程不断**。

## 手势

投屏页不放"返回/主页/多任务"按钮 —— 直接在画面里比划即可,由被控手机自己的手势导航响应。为免本机系统手势抢走边缘滑动,被控分辨率不小于本机时画面四周会留一圈空白:**空白区归本机,画面区归被控手机**,画面区内还会调用 `setSystemGestureExclusionRects` 屏蔽本机手势。
